    ...
    def alloc_net_resources(self, net, route, o, d, w, ht, cr, gr):
        ...
        for r in range(length-1):
            ...
            if not gr:
                ...
                for n in range(r+2,length):
                    key_c = (rcurr, route[n-1], w)
                    route_c = route[r:n]
...
class DijkstraFirstFit(RWAAlgorithm):
    ...
    def rwa(self, net, orig, dest, hold_t, call_req, time):  # call_req : call request bandwidth requirement
        """ This method RWAs """
...
class DijkstraFirstFitWithGrooming(RWAAlgorithm):
    ...
    def rwa(self, net, orig, dest, hold_t, call_req,time):  # call_req : call request bandwidth requirement 
        ...
        lam = util(SIM_LOAD,SIM_NUM_CALLS)
        gStartTime = net.num_channels/lam - np.sqrt(net.num_channels)/(2*lam)
        gEndTime = net.num_channels/lam + np.sqrt(net.num_channels)/(2*lam)
        ...
        if time>=gStartTime and time<= gEndTime:
            #Stateful Grooming
...
class GeneticAlgorithm(RWAAlgorithm, Environment):
    ...
    def rwa(self, net, orig, dest, hold_t, call_req, time):
